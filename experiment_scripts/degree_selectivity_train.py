...
def main():
    ...
    for iteration in range(10):
        for keep_exponent in range(1,25):
            ...
            degree_centrality = unit_graph.degree(weight='weight')

            dense_300_centrality = np.asarray([degree_centrality['dense_300.'+str(i)] for i in range(300)])
            dense_300_centrality = minmax(dense_300_centrality)
            dense_100_centrality = np.asarray([degree_centrality['dense_100.'+str(i)] for i in range(100)])
            ...
            model.save('../models/trained/degree_selectivity_percent_'+str(keep_exponent)+'_iteration_'+str(iteration))
            json.dump(history,open('../histories/degree_selectivity_percent_'+str(keep_exponent)+'_iteration_'+str(iteration)+'.json','w'))

            best_epoch = np.argmin(history['val_loss'])
            stats.loc[len(stats)] = [iteration,'degree_selectivity',percent_to_keep,None,(num_kept_mask_300,num_kept_mask_100),best_epoch,history['val_loss'][best_epoch],history['val_accuracy'][best_epoch]]
            stats.to_csv('../summary_stats/lenet_degree_selectivity.csv')
